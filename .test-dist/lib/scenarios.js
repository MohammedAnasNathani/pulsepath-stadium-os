...
exports.normalizeVenueState = normalizeVenueState;
const baseZones = [
    {
        zoneId: "north-gate",
        label: "North Gate",
...
const scenarioTemplates = {
    "pre-entry-rush": {
        label: "Pre-entry Rush",
        narrative: "PulsePath senses an uneven gate load. North Gate is swelling, but East Gate and the Fan Plaza give attendees a faster, calmer path into the bowl.",
    ...
    "halftime-surge": {
        label: "Halftime Surge",
        narrative: "The whole venue pivots at halftime. Food, restrooms, and merch all spike together, so the assistant has to protect return-to-seat timing as much as it protects convenience.",
        matchLabel: "Mumbai Meteors vs Bengaluru Blaze",
    ...
    "accessible-reroute": {
        label: "Accessible Reroute",
        narrative: "One vertical transport core is running slow, so PulsePath has to protect dignity and confidence for mobility-sensitive guests with staff handoffs and calmer corridors.",
        matchLabel: "Mumbai Meteors vs Bengaluru Blaze",
    ...
    "emergency-exit": {
        label: "Emergency Exit Coordination",
        narrative: "A localized smoke alert in the west bowl means the venue must issue confident reroutes, avoid panic, and move fans through safe exit spines with clarity.",
        matchLabel: "Mumbai Meteors vs Bengaluru Blaze",
...
function buildMetrics(zones, incidents) {
    const averageWait = Math.round(zones.reduce((sum, zone) => sum + zone.waitMins, 0) / zones.length) || 0;
    const peakDensity = Math.max(...zones.map((zone) => zone.density));
    const accessFriendly = Math.round(zones.reduce((sum, zone) => sum + zone.accessibilityScore, 0) / zones.length);
    return [
        {
            key: "wait",
            label: "Avg Wait",
...
function normalizeVenueState(candidate) {
