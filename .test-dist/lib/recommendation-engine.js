...
exports.createOpsAnnouncement = createOpsAnnouncement;
exports.buildDeterministicRecommendation = buildDeterministicRecommendation;
exports.buildAssistantResponse = buildAssistantResponse;
exports.buildRecommendationRequestFromScenario = buildRecommendationRequestFromScenario;
...
function zoneIndex(state) {
    return new Map(state.zones.map((zone) => [zone.zoneId, zone]));
}
function buildReasons(profile, zone, state) {
    const reasons = [
    ...
    const matchingIncident = state.incidents.find((incident) => incident.affectedZones.includes(zone.zoneId));
    if (matchingIncident) {
        reasons.push(matchingIncident.rerouteBias);
    }
    return reasons;
}
function buildRouteSteps(currentZone, option, zone, state) {
    const steps = [
        {
            label: "Start",
...
function zoneLabelFor(state, zoneId) {
    return state.zones.find((zone) => zone.zoneId === zoneId)?.label ?? zoneId;
}
function scoreCandidate(profile, option, zone, state) {
    let score = 140;
...
function buildHeadline(goal, topAction, state) {
    const prefixes = {
...
function buildFallbackReasoning(profile, state, actions) {
...
function createOpsAnnouncement(state, topAction) {
    ...
    if (incident?.severity === "critical") {
        return `Attention guests: please move calmly away from ${incident.affectedZones
            ...
            .join(" and ")} and follow staff guidance toward ${zoneLabelFor(state, topAction?.destinationZoneId ?? "south-exit")}.`;
    }
...
function buildDeterministicRecommendation(request) {
...
async function createGeminiReasoning(request, deterministic, fetchImpl) {
...
async function buildAssistantResponse(request, options = {}) {
    const deterministic = buildDeterministicRecommendation(request);
    if (options.disableAi) {
        return deterministic;
    }
    const aiReasoning = await createGeminiReasoning(request, deterministic, options.fetchImpl ?? fetch).catch(() => null);
    if (!aiReasoning) {
        return deterministic;
    }
    return {
...
function buildRecommendationRequestFromScenario(profile, currentZone, scenarioId) {
    const venueState = (0, scenarios_1.getScenarioState)(scenarioId);
    return {
        profile,
        currentZone,
